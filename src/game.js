...
export async function game() {
    ...
    playerSprite.forever(() => {
        ...
        if (room && room.soundName === 'desk') {
            if (player.stats.burnout < 70 && player.stats.social > 30) {
                player.stats.utilisation = (player.stats.utilisation || 0) + 0.1;
            }
        }
    ...
    ws.onmessage = function incoming(message) {
        ...
        loggedOff.forEach((id) => {
            ...
            playerSprites[id].utilisationLabel.destroy();
...
function renderPlayerStats(player) {
    const { burnout, social, utilisation } = player.stats || { burnout: 0, social: 0 };
    ...
    player.utilisationLabel = player.utilisationLabel || new Text;
